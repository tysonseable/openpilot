...
def create_lat_ctl_msg(
    ...
    values = {
        ...
        "LatCtl_D_Rq": 1
        if lat_active
        else 0,  # Mode: 0=None, 1=ContinuousPathFollowing, 2=InterventionLeft,
...
def create_acc_msg(
    packer, CAN: CanBus, long_active: bool, gas: float, accel: float, stopping: bool
):
    """
    ...
    values = {
        ...
        "AccBrkPrchg_B_Rq": 1
        if newDecelValue
        else 0,  # Pre-charge brake request: 0=No, 1=Yes
        "AccBrkDecel_B_Rq": 1
        if newDecelValue
        else 0,  # Deceleration request: 0=Inactive, 1=Active
        "AccStopStat_B_Rq": 1 if stopping else 0,
    }
    return packer.make_can_msg("ACCDATA", CAN.main, values)


def create_acc_ui_msg(
    packer,
    CAN: CanBus,
    CP,
    main_on: bool,
        ...
        values.update(
            {
                ...
                "AccFllwMde_B_Dsply": 1
                if hud_control.leadVisible
                else 0,  # Lead indicator
...
def create_button_msg(
    packer, bus: int, stock_values: dict, cancel=False, resume=False, tja_toggle=False
):
    """
